    ...
    if (err) {
      ...
      pool.getConnection(function (err, con) {
        if (err) {
          ...
          sql = `select property_images from place_info where place_id='${property_id}'`
          con.query(sql, function (err, result) {
            if (err) {
              res.writeHead(400, {
                'Content-Type': 'text/plain'
              ...
              sql = `update place_info set property_images=concat(ifnull(property_images,""),'${property_images}') where place_id='${property_id}'`;
              console.log(sql);
              con.query(sql, function (err, result) {
                if (err) {
                  res.writeHead(400, {
