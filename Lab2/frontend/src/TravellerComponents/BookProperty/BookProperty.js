...
class BookProperty extends Component {
    ...
    bookPlace = (e) => {
        ...
        console.log(this.props.location.state && this.props.location.state.response._id)
        axios.post("http://localhost:3001/property/" + data._id + "/book", data)
            .then(response => {
                console.log("Got the data from post request to localhost")
                if (response.status === 200) {
                    this.setState({
                        bookingConfirmation: "Booked"
                    })
                }
            })
    }
